...
【特徴・設計方針】
- 入力: BGR画像 (uint8, 0-255, 固定解像度) → CoreML側で0-1正規化
- 出力: BGR画像 (float, 0-255, 固定解像度の4倍)
...
【利用例】
python scripts/pytorch2coreml.py --pth weights/RealESRGAN_x4plus.pth --out weights/RealESRGAN_x4plus.mlpackage --input-size 256 --float16
"""
...
def validate_args(args: argparse.Namespace) -> Optional[str]:
    ...
    if args.input_size <= 0:
        return f"入力サイズは正の値である必要があります（指定値: {args.input_size}）"
    if args.input_size % 4 != 0:
        return f"入力サイズは4の倍数である必要があります（指定値: {args.input_size}）"
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="入力画像の解像度（正方形、4の倍数、デフォルト: 256）",
    )
    parser.add_argument(
        "--scale",
        type=int,
    ...
    try:
        ...
        output_size = input_size * args.scale

        input_desc = ct.ImageType(
            name="input_image",
            shape=(1, 3, input_size, input_size),  # 固定解像度
        ...
        print(f"[INFO] モデルをトレース中: 入力サイズ {input_size}x{input_size}")
        ...
        print(f"[INFO] 入力shape: (1,3,{input_size},{input_size}) 固定解像度")
        print(f"[INFO] 出力shape: (1,3,{output_size},{output_size}) 固定解像度")
        ...
        print(f"[INFO] 入力解像度: {input_size}x{input_size}")
        print(f"[INFO] 出力解像度: {output_size}x{output_size}")
